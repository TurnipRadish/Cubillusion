...
data modify storage bs:data load.modules append value {module:"bs.load",version:"3.0.0"}
...
execute if score $bs.load.major load.status matches 3 if score $bs.load.minor load.status matches 0 unless score $bs.load.patch load.status matches ..0 run return 0
